...
app.post('/api/auth/inscription', async (req, res) => {
  try {
    ...
    const result = await client.query(
      ...
      [email, hashedPassword, nom, telephone, 0.00]
    );
...
app.post('/api/auth/connexion', async (req, res) => {
  try {
    ...
    const result = await client.query(
      'SELECT id, email, password, nom, telephone, solde FROM users WHERE email = $1',
      [email]
    );
...
app.post('/api/transaction/:id/payer', authentifierToken, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      'UPDATE users SET solde = solde - $1, updated_at = NOW() WHERE id = $2',
...
app.post('/api/recharger', authentifierToken, async (req, res) => {
  try {
    const { montant, operateur, numeroTelephone } = req.body;
    const userId = req.user.userId;
    ...
    // Simuler une recharge (dans la réalité, intégrer avec les APIs MTN/Orange)
    const result = await client.query(
      `INSERT INTO recharges (user_id, montant, operateur, numero_telephone, statut)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [userId, parseFloat(montant), operateur, numeroTelephone, 'valide']
    );
    ...
    res.json({
      ...
      message: `Rechargement de ${montant}FCFA réussi via ${operateur}`
    });
  } catch (error) {
...
app.get('/api/historique', authentifierToken, async (req, res) => {
  try {
    ...
    const historique = result.rows.map(transaction => ({
      id: transaction.id,
      montant: parseFloat(transaction.montant),
      boissons: transaction.boissons,
      statut: transaction.statut,
...
app.get('/api/solde', authentifierToken, async (req, res) => {
  try {
    const client = await pool.connect();
    ...
    res.json({ success: true, solde: solde });
  } catch (error) {
...
async function demarrerServeur() {
  try {
    ...
    app.listen(PORT, '0.0.0.0', () => {
      console.log(`🚀 Serveur backend avancé démarré sur le port ${PORT}`);
